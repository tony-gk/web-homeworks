...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method beforeMethod = findPageMethod(pageClass, "before");
        Method afterMethod = findPageMethod(pageClass, "after");
        Method actionMethod = findPageMethod(pageClass, route.getAction());

        if (actionMethod == null || afterMethod == null || beforeMethod == null) {
            throw new NotFoundException();
        }
        ...
        try {
            ...
            ArrayList<Object> objectsToInvoke = new ArrayList<>();
            for (Class<?> clazz : actionMethod.getParameterTypes()) {
                if (clazz.equals(Map.class)) {
                    objectsToInvoke.add(view);
                } else {
                    objectsToInvoke.add(request);
                }
            }

            beforeMethod.invoke(page, request, view);
            actionMethod.invoke(page, objectsToInvoke.toArray());
    ...
    private Method findPageMethod(Class<?> pageClass, String methodName) {
        Method pageMethod = null;
        for (Class<?> clazz = pageClass; pageMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
            pageMethod = getActionByName(clazz, methodName);
        }
        return pageMethod;
    }


    private Method getActionByName(Class<?> pageClass, String methodName) {
        Object[] methods = Arrays.stream(pageClass.getDeclaredMethods()).filter(method ->
                method.getName().equals(methodName)
                        && method.getReturnType().equals(void.class)
                        && method.getParameterCount() <= 2
                        && Arrays.stream(method.getParameterTypes())
                        .allMatch(clazz -> clazz == HttpServletRequest.class || clazz == Map.class)).toArray();
        if (methods.length == 0) {
            return null;
        } else {
